...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
    ...
    float noise = tex2D(baseTexture, coords * 0.3 + float2(coords.y, -localTime));
    float3 color = baseColor * lerp(0.5, 1, noise);
    
    // Apply additive blending.
    float additiveNoise = tex2D(baseTexture, (coords + float2(coords.y + noise * 0.5, -localTime)) * 0.85);
