...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
    ...
    innerGlow += 0.12 / distanceFromCenter;
    float4 glow = innerGlow;
    
    float opacity = smoothstep(0.55, 0.34, distanceFromCenter) * smoothstep(0, 0.04, coords.x - saturate(innerGlow) * 0.014);
    color += glow * color.a;
