...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    float generalNoise = sin(polar.y * 6.34589 - globalTime * 1.4 + angleFromCenter * 10) * sin(polar.y * -11.07211 + globalTime * 0.4815 + angleFromCenter * -5) * 0.5 + 0.5;
    float glowNoise1 = tex2D(noiseTexture, polar * float2(0.5, 2) + float2(globalTime * 0.3, 0.05 - globalTime * 0.25) - generalNoise * 0.04);
    float glowNoise2 = tex2D(noiseTexture, polar * float2(0.5, 7) + float2(-globalTime * 0.3, 0.29 - globalTime) + generalNoise * 0.025);
    ...
    float4 color = pow(float4(sampleColor.rgb, 1) / (radiusInterpolant + 0.2), 1.25) + smoothstep(0.6, 0.99, radiusInterpolant) + glowNoise - float4(1, 1, 1, 0) * (1 - radiusInterpolant);
    color += smoothstep(0.45, 0.5, distanceFromCenter) * 1.6;
